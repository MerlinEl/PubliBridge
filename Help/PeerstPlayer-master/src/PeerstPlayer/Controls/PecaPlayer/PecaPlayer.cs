...
namespace PeerstPlayer.Controls.PecaPlayer
{
	...
	public partial class PecaPlayerControl : UserControl
	{
		...
		public void OpenTest(string streamUrl) {
			// 再生中の判定
		...
		public void Open(string streamUrl)
		{
			...
			// Preparing for PeerCast communication
			...
			updateChannelInfoWorker.DoWork += (sender, e) =>
			{
				...
				{
					Logger.Instance.DebugFormat("Channel update try [{0} times] ", i+1);
				...
				Logger.Instance.Debug("Channel update failed.");
				e.Result = false;
			};
			updateChannelInfoWorker.RunWorkerCompleted += (sender, e) =>
			{
				if ((bool)e.Result)
				{
					Logger.Instance.InfoFormat("Channel update completed [Channel name: {0}] [Genre: {1}] [Details: {2}] [Comment: {3}] [Contact URL: {4}]", ChannelInfo.Name, ChannelInfo.Genre, ChannelInfo.Desc, ChannelInfo.Comment, ChannelInfo.Url);
		...
		{
			Logger.Instance.DebugFormat("RetryPlayer()");
			moviePlayer.Retry();
		}
