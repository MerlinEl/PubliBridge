	...
	public partial class WriteFieldControl : UserControl
	{
		...
		public WriteFieldControl()
		{
			...
			writeResWorker.DoWork += (sender, e) =>
			{
				...
				try
				{
					Logger.Instance.InfoFormat("Write less [Bulletin board: {0} Thread: {1}, Body: {2}]", operationBbs.BbsInfo.BbsName, operationBbs.SelectThread.ThreadTitle, message);

					operationBbs.Write("", "sage", message);
					Logger.Instance.InfoFormat("Less write: Success [Bulletin board: {0} Thread: {1}, Body: {2}]", operationBbs.BbsInfo.BbsName, operationBbs.SelectThread.ThreadTitle, message);
				}
				catch (Exception ex)
				{
					result.WriteResult = false;
					result.Exception = ex;
					Logger.Instance.ErrorFormat("Less write: Failure [Error content {0} Bulletin board: {1} Thread: {2}, Body: {3}]", ex.Message, operationBbs.BbsInfo.BbsName, operationBbs.SelectThread.ThreadTitle, message);
				}
				finally
				{
					e.Result = result;
			...
			writeResWorker.RunWorkerCompleted += (sender, e) =>
			{
				...
				writeFieldTextBox.WatermarkText = "Please enter the content to be written";

				// 書き込み成功
				if (result.WriteResult)
				{
					ToastMessage.Show("Less writing is completed.");
					writeFieldTextBox.Text = "";
				}
				// 書き込み失敗
				else
				{
					ToastMessage.Show(string.Format("Failed to write less. [{0}]", result.Exception.Message));
					writeFieldTextBox.Text = result.Message;
					Logger.Instance.ErrorFormat("{0}\n\nBulletin board: {1}\n Thread: {2}\n Body: {3}", result.Exception.Message, operationBbs.BbsInfo.BbsName, operationBbs.SelectThread.ThreadTitle, result.Message);
				}
		...
		//-------------------------------------------------------------
		// Summary: Less writing
		//-------------------------------------------------------------
		private void WriteRes()
		{
			if (!writeResWorker.IsBusy)
			{
				ToastMessage.Show("Writing ...");
