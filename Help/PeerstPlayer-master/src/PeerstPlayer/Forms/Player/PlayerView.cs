...
namespace PeerstPlayer.Forms.Setting
{
	...
	public partial class PlayerView : GlowForm
	{
		...
		public PlayerView()
		{
			...
			{
				Logger.Instance.InfoFormat("Command line arguments [{0}]", arg);
			}
			...
			if (Environment.GetCommandLineArgs().Length > 3)
			{
				...
				Logger.Instance.InfoFormat("Channel name: {0}", name);
			...
			{
				Logger.Instance.InfoFormat("Screen display --Shown event start");
				...
				if (Environment.GetCommandLineArgs().Length > 1)
				{
					...
					Logger.Instance.InfoFormat("Video playback [url: {0}]", url);
					Open(url);
				}
		...
		private void InitEvent()
		{
			...
			timer.Tick += (sender, e) =>
			{
				// Display switching of automatic display button
				if (toolStrip.Visible && !IsVisibleToolStrip(PointToClient(MousePosition)))
				{
					toolStrip.Visible = false;
				}

				// Mouse gestures are canceled when you go off the screen
				...
				if (pecaPlayer.Duration.Length == 0)
				{
					switch (pecaPlayer.PlayState)
					{
						...
						case WMPPlayState.wmppsStopped: statusBar.MovieStatus		= "Stop";		break;
						case WMPPlayState.wmppsPaused: statusBar.MovieStatus		= "Stop for a while";	break;
						case WMPPlayState.wmppsPlaying: statusBar.MovieStatus		= "Regenerating";		break;
						case WMPPlayState.wmppsScanForward: statusBar.MovieStatus	= "Fast forward";		break;
						case WMPPlayState.wmppsScanReverse: statusBar.MovieStatus	= "Rewind";	break;
						case WMPPlayState.wmppsWaiting: statusBar.MovieStatus		= "Waiting for connection";	break;
						case WMPPlayState.wmppsMediaEnded: statusBar.MovieStatus	= "Regenerated";	break;
						case WMPPlayState.wmppsTransitioning: statusBar.MovieStatus = "Preparing";		break;
						case WMPPlayState.wmppsReady: statusBar.MovieStatus			= "Ready";	break;
						case WMPPlayState.wmppsReconnecting: statusBar.MovieStatus	= "Reconnect";		break;
						case WMPPlayState.wmppsBuffering: statusBar.MovieStatus		= string.Format("Buffer {0}%", pecaPlayer.BufferingProgress); break;
					}
				}
				// 再生時間を表示
				else
			...
			pecaPlayer.MouseMoveEvent += (sender, e) =>
			{
				...
				if (isGesturing)
				{
					...
					if (!String.IsNullOrEmpty(gesture))
					{
						string message = string.Format("Gesture: {0} {1}", gesture, (String.IsNullOrEmpty(detail) ? "" : "(" + detail + ")"));
			...
			// Displaying command execution details
			shortcut.CommandExecuted += (sender, e) =>
			{
				CommnadExecutedEventArgs args = (CommnadExecutedEventArgs)e;

				// Status bar display switching does not output a message
				if (args.Command != Commands.VisibleStatusBar)
				{
					ToastMessage.Show(string.Format("Command: {0}", args.Detail));
				}
			};
			...
			loadSWFTestToolStripMenuItem.Click += (sender, e) => shortcut.ExecCommand(Commands.LoadSWF);
			...
			openFromClipboardToolStripMenuItem.Click += (sender, e) =>
			{
				...
				{
					MessageBox.Show("Failed to open the clipboard");
				}
			};
