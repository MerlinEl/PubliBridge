...
namespace PeerstPlayer.Shortcut
{
	...
	public class ShortcutManager
	{
		...
		/// <summary>
		/// Command Map (command ID-> get command class)
		/// </summary>
		...
		public void Init(PlayerView playerView, PecaPlayerControl pecaPlayer, StatusBarControl statusBar)
		{
			...
			{
				Logger.Instance.Error("The default settings are loaded because the shortcut setting file failed to be loaded.");
				LoadDefaultSettings();
			}
			...
			{
				Logger.Instance.Error("The shortcut settings file is corrupted, so load the default settings.");
				LoadDefaultSettings();
			}
		}
		...
		{
			Logger.Instance.InfoFormat("Event execution [Event ID: {0}]", eventId);
			Commands commands;
			...
			{
				Logger.Instance.ErrorFormat("There is no command associated with the event [eventid: {0}]", eventId);
			}
		}
		...
		public void RaiseKeyEvent(AxWMPLib._WMPOCXEvents_KeyDownEvent e)
		{
			...
			Logger.Instance.InfoFormat("Key press event execution [Event ID: {0}, keyCode {0}, modifierKey {1}] ", keyCode, modifierKey);
		...
		public void ExecGesture(string gesture)
		{
			...
			{
				Logger.Instance.InfoFormat("Mouse Gesture Execution [Gesture: {0}, Command ID: {1}]", gesture, commands);
				ExecCommand(commands);
			}
		}
		...
		{
			Logger.Instance.InfoFormat("Command execution [command ID: {0}]", commands);
			...
			// Command execution event-> Notify execution contents
		...
		private void CreateCommand(Form form, PecaPlayerControl pecaPlayer, StatusBarControl statusBar)
		{
			commandMap = new Dictionary<Commands, ShortcutCommand>()
			{
				...
				{	Commands.LoadSWF,				new ShortcutCommand(new ScreenshotCommand(pecaPlayer), new CommandArgs())							}, // スクリーンショット
		...
		private void SettingKey()
		{
			...
			settings.KeyMap.Add(new KeyInput(Keys.Control, Keys.Left), Commands.VolumeBalanceLeft);
			settings.KeyMap.Add(new KeyInput(Keys.Control, Keys.Down), Commands.VolumeBalanceMiddle);
			settings.KeyMap.Add(new KeyInput(Keys.Control, Keys.Right), Commands.VolumeBalanceRight);
