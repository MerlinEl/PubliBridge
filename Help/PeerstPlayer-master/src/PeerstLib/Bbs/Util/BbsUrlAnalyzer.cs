...
namespace PeerstLib.Bbs.Util
{
	...
	public static class BbsUrlAnalyzer
	{
		...
		public static BbsInfo Analyze(string threadUrl)
		{
			...
			{
				Logger.Instance.WarnFormat("The URL is invalid. [uri:{0}]", threadUrl);
				return new BbsInfo
				{
					Url = "",
			...
			{
				Logger.Instance.DebugFormat("Bulletin board server: Taraba [HOST:{0}]", uri.Host);
			...
			else
			{
				...
				{
					Logger.Instance.DebugFormat("Bulletin board server: 2ch compatible server [HOST:{0}]", uri.Host);
					return info;
				}
			}

			// 未対応
			Logger.Instance.DebugFormat("Bulletin board server: Not supported [HOST:{0}]", uri.Host);
			return new BbsInfo
			{
				Url = threadUrl,
				BbsServer = BbsServer.UnSupport,
