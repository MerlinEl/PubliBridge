...
namespace Interop.ShockwaveFlashObjects {
    ...
    public delegate void IIShockwaveFlashEvents_FlashCallEventHandler([MarshalAs(UnmanagedType.BStr)] string request);
    ...
    // Event interface for Shockwave Flash
    [ComEventInterface(typeof(IIShockwaveFlashEvents), typeof(IIShockwaveFlashEvents_EventProvider))]
    [ComVisible(false)]
    public interface IIShockwaveFlashEvents_Event {
        ...
        event IIShockwaveFlashEvents_FlashCallEventHandler FlashCall;
    }

    [ClassInterface(ClassInterfaceType.None)]
    internal class IIShockwaveFlashEvents_SinkHelper:IIShockwaveFlashEvents {
        public int Cookie = 0;
        ...
        void IIShockwaveFlashEvents.OnReadyStateChange(int newState) {
        ...
        void IIShockwaveFlashEvents.OnProgress(int percentDone) {
        ...
        void IIShockwaveFlashEvents.FSCommand(string command, string args) {
            FSCommandDelegate?.Invoke(command, args);
        }

        public event IIShockwaveFlashEvents_FlashCallEventHandler FlashCallDelegate = null;
        public void Set_FlashCallDelegate(IIShockwaveFlashEvents_FlashCallEventHandler deleg) {
            FlashCallDelegate = deleg;
        }
        public bool Is_FlashCallDelegate(IIShockwaveFlashEvents_FlashCallEventHandler deleg) {
            return ( FlashCallDelegate == deleg );
        }
        public void Clear_FlashCallDelegate() {
            FlashCallDelegate = null;
        }
        void IIShockwaveFlashEvents.FlashCall(string request) {
            FlashCallDelegate?.Invoke(request);
        }
    }

    internal class IIShockwaveFlashEvents_EventProvider:IDisposable, IIShockwaveFlashEvents_Event {
        IConnectionPointContainer ConnectionPointContainer;
        IConnectionPoint ConnectionPoint;
        IIShockwaveFlashEvents_SinkHelper EventSinkHelper;
        int ConnectionCount;

        // Constructor: remember ConnectionPointContainer
        //IIShockwaveFlashEvents_EventProvider(object CPContainer) : base() {
        //    ConnectionPointContainer = ( IConnectionPointContainer )CPContainer;
        //}

        // Force disconnection from ActiveX event source
        ~IIShockwaveFlashEvents_EventProvider() {
            Disconnect();
        ...
        void Connect() {
            if ( ConnectionPoint == null ) {
                ...
                EventSinkHelper = new IIShockwaveFlashEvents_SinkHelper();
        ...
        event _IShockwaveFlashEvents_OnReadyStateChangeEventHandler IIShockwaveFlashEvents_Event.OnReadyStateChange {
            add {
                System.Threading.Monitor.Enter(this);
                try {
                    Connect();
        ...
        event _IShockwaveFlashEvents_OnProgressEventHandler IIShockwaveFlashEvents_Event.OnProgress {
            add {
                System.Threading.Monitor.Enter(this);
                try {
                    Connect();
        ...
        event _IShockwaveFlashEvents_FSCommandEventHandler IIShockwaveFlashEvents_Event.FSCommand {
            add {
                System.Threading.Monitor.Enter(this);
                try {
                    Connect();
        ...
        event IIShockwaveFlashEvents_FlashCallEventHandler IIShockwaveFlashEvents_Event.FlashCall {
            add {
                System.Threading.Monitor.Enter(this);
                try {
                    Connect();
